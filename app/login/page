...
export default function LoginPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      document.cookie = `chu_token=${token}; path=/; max-age=2592000; SameSite=Lax`
      document.cookie = `chu_user=${encodeURIComponent(JSON.stringify({ id: user.id, email: user.email, name: fullName }))}; path=/; max-age=2592000; SameSite=Lax`
        ...
        <div style={{width:'100%',maxWidth:400}}>
          {isMobile && (
            <Link href="/" style={{display:'flex',alignItems:'center',gap:8,marginBottom:24,textDecoration:'none',justifyContent:'center'}}>
              <div style={{width:32,height:32,borderRadius:8,background:'#0891b2',display:'flex',alignItems:'center',justifyContent:'center',fontSize:14,fontWeight:800,color:'#fff'}}>{{PRODUCT_NAME_INITIAL}}</div>
